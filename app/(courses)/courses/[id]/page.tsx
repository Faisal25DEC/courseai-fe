...
const PreivewCourse = () => {
  ...
  const checkIfLessonIsLocked = (idx: number) => {
    ...
    for (let j = idx - 1; j >= 0; j--) {
      const lessonId = filteredLessonsArray[j].id;
    ...
    console.log(`Lesson ${filteredLessonsArray[idx].id} is not locked.`);
    return false;
  };
  ...
  const areAllLessonsLocked = () => {
    console.log(
      "areAllLessonsLocked ",
      !filteredLessonsArray.every((lesson: any, idx: any) =>
        checkIfLessonIsLocked(idx)
      )
    );
    return !filteredLessonsArray.every((lesson: any, idx: any) =>
      checkIfLessonIsLocked(idx)
    );
  };
  ...
  const getLockedLessons = (lessonsArray: any) => {
    const lockedLessons = lessonsArray.map(
  ...
  const filteredLessons = getLockedLessons(filteredLessonsArray);
      ...
      <div className="flex h-full w-[100%]">
        {showContent && (
          <div className="w-[40%] border-r-[1px] h-full overflow-auto border-r-gray-200 flex flex-col my-element bg-white">
            ...
            {filteredLessonsArray.map((lesson: any, idx: any) => (
              <div
                onClick={() => handleChangeLesson(idx)}
                key={lesson.id}
                style={{ opacity: lesson.locked ? 0.5 : 1 }}
                className={`flex cursor-pointer items-start relative justify-between duration-200 transition-all ease-linear px-4 py-4 text-white border-b-1 border-gray-200 ${
                  activeLesson === idx
        ...
        <div className="flex flex-col w-full px-4">
          ...
          <div className="overflow-auto w-full pr-5 py-2 border-1 rounded-lg bg-white my-4 my-element">
            ...
            {filteredLessonsArray[activeLesson]?.type === "video" && (
              <VideoLesson
                video={filteredLessonsArray[activeLesson].content}
                lesson={filteredLessons[activeLesson]}
              />
            )}
            {filteredLessonsArray[activeLesson]?.type === "text" && (
              <TextLesson
                content={filteredLessonsArray[activeLesson].content}
                lesson_id={filteredLessonsArray[activeLesson].id}
                lesson={filteredLessons[activeLesson]}
              />
            )}
            {filteredLessonsArray[activeLesson]?.type === "avatar" && (
              <AvatarLesson
                lesson={filteredLessons[activeLesson]}
                voice_id={filteredLessonsArray[activeLesson].content?.voice_id}
                avatar_id={filteredLessonsArray[activeLesson].content?.avatar_id}
                thumbnail={
                  filteredLessonsArray[activeLesson].content?.avatar
                    ?.normal_thumbnail_medium
                }
                avatar_name={filteredLessonsArray[activeLesson].content?.avatar?.id}
                lesson_id={filteredLessonsArray[activeLesson].id}
