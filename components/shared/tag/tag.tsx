import { color, textFromBg } from "@/lib/constants";
import React, { CSSProperties, ReactNode } from "react";

function alternateDarkColor(hexColor: string, darknessFactor: number): string {
...
function darkenHexColor(hex: string | undefined, percent: number): string {
  if (!hex) return "#000000"; // Return black if hex is undefined
...
function hexToDarkHex(hex: string): string {
  hex = hex.replace("#", "");
  const r = parseInt(hex.substring(0, 2), 16);
  const g = parseInt(hex.substring(2, 4), 16);
  const b = parseInt(hex.substring(4, 6), 16);
...
const randomColor: any = color[Math.floor(Math.random() * color.length)];
...
const Tag: React.FC<TagProps> = ({ bg, color, children, ...rest }) => (
  <div
    {...rest}
    style={{
